...
pub const SendQueue = struct {
    ...
    fn reclaimMemory(this: *SendQueue) void {
        ...
        // Keep a minimum capacity to avoid frequent reallocations for small messages.
        const min_capacity = 4096;
        const shrink_threshold = 16384; // Only shrink if capacity exceeds this
        ...
        // Shrink the send queue if it has excess capacity
        if (this.queue.capacity > shrink_threshold and this.queue.items.len < this.queue.capacity / 4) {
